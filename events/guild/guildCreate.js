module.exports = (Discord, client, guild) => {
    ...
    const embedMsg = new Discord.EmbedBuilder()
        ...
        .setDescription(
            "The-Bot contains a special muting system where members of the server can be muted by using ``!mute`` command if they broke any rule. This will add a new role to the mentioned member and they cannot speak in the server. So to make this possible the permissions of the roles are based on @everyone role so make sure you disabled all the permissions expect VIEW_CHANNEL permission. But this feature is still in development. This is so important. We are trying to make it automatically it might available from next update.\n\n Thank you"
        )
