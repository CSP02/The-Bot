		...
		try {
			...
			await mongo().then(async mongoose => {
				try {
					...
					await msg.channel.send('Connection successfull!\nSearching the mentioned Infractions.\n')
						...
						if (isInfrId) {
							if ((athr == ' ' || resn == ' ' || target == ' ')) {
								...
								const embdmsg = new Discord.EmbedBuilder()
									.setTitle('Infraction')
									.setColor('#ff0000')
									.setFooter({ text: infrId, icon_url: '../../Images/infr.png' })
									.addFields(
										[{ name: "Author", value: `${athr}` },
										{ name: 'User', value: `${target}` },
										{ name: 'Infraction Type:', value: `${infrTp}` },
										{ name: 'Reason', value: `${resn != '' ? resn : 'unsspecified'}` },]
									)
							...
							if (infrId == ' ' || target == ' ') {
								...
								const embdmsg = new Discord.EmbedBuilder()
									.setTitle('Infraction')
									.setColor('#ff0000')
									.setFooter({ text: `use !infr <infrID> to get more info about the infraction.` })
									.addFields(
										[{ name: 'User', value: `${target}` },
