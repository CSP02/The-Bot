	...
	async execute(client, message, args, Discord) {
		try{
			...
			if (isNaN(args[0]))
				...
				target = await message.guild.users.fetch(args[0])
			if (target) {
				if (args[1]) {
					const memberTarget = message.guild.members.cache.get(target.id);
					if (memberTarget.permissions.has(PermissionsBitField.Flags.KickMembers)) { message.reply("You can't do that, but nice try though.") }
					else {
						memberTarget.kick();
						...
						const embedMsg = new Discord.EmbedBuilder()
							...
							.addFields(
								[{ name: 'Reason:', value: `${args.slice(1).join(" ") != '' ? args.slice(1).join(" ") : 'unspecified'}` }]
							)
